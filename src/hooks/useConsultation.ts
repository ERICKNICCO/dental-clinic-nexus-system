...
export const useConsultation = (patientId: string) => {
  ...
  const loadActiveConsultation = useCallback(async () => {
    try {
      setLoading(true);
      const consultation = await consultationService.getActiveConsultation(patientId);
  ...
  const startConsultation = async (doctorId: string, doctorName: string, appointmentId?: string) => {
    try {
      const consultation = await consultationService.startConsultation(patientId, doctorId, doctorName, appointmentId);
      setActiveConsultation(consultation);
      return consultation;
    } catch (error) {
  ...
  const completeConsultation = async (consultationId: string, finalData: Partial<Consultation>) => {
    try {
      await consultationService.completeConsultation(consultationId, finalData);
      setActiveConsultation(null);
    } catch (error) {
